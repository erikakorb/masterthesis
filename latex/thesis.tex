...
The formation pathaways of the black hole binaries are an active field of research and can be divided into two main evolutionary scenarios: isolated binary or dynamically active environments. Different formation histories can be investigated mainly by the analysis of the features in the distribution of the masses, spins and related quantities: different formation channels lead to different predictions on the black hole masses, mass ratio $q=M_2/M_1$ ($M_1 \geq M_2$), spin alignment and magnitude. 
 
On the one hand, the isolated binary evolution assumes that tidal interactions and mass transfer episodes are so efficient to produce compact objects with similar masses, i.\ e. $q \sim 1$ \cite{giacobbomapelli2018_mobse_fryer} with spins aligned and parallel to the orbital angular momentum \cite{Kalogera2000_spinaligned}. On the other hand
...
Even though the region $\gtrsim 70~\msun$ is still weakly explored, there is no strong evidence for or against a mass-gap, at least up to $\sim 75~\msun$. Still, a similar finding challenges the existence of the pair-instability mass gap from $\sim 60~\msun$ to $\sim 120~\msun$ \cite{spera2017_pisnSNe}: either the single stellar evolution models need to be corrected (for instance, by lowering the triple-$\alpha$ reaction rate it is possible to shift to higher masses the lower boundary \cite{MassGapStellarEvo_Costa2021}) or black holes in that mass range (like the $\sim 69~\msun$ and $\sim 95~\msun$ black holes of GW190521 \cite{GWTC-2}) are not formed in the isolated binary scenario but, for instance, as the result of hierarchical merging in a dynamically active environment.\cite{Rastello2021_dynamics} On the other hand, there is still evidence of a lower-mass gap between $\sim 3~\msun$ and $\sim 5~\msun$, potentially due to the physics of core-collapse supernovae and in agreement with the dearth of compact objects observed in the galactic X-ray binaries \cite{massgapreal_ozel2010}.
...
On the one hand, the spin magnitude of the black holes in binary systems is expected to be influenced by the efficiency of the angular momentum transport in the stellar progenitors and by the binary processes like tides and mass transfer. On the other hand, the spin orientations are considered as proxy for the evolutionary channels: binary black holes formed from isolated binaries are expected to have spins aligned with their orbital angular momentum whereas dynamically assembled binaries have more likely isotropically oriented spins. Unfortunately, up-to-date it is still difficult to use the spins to discriminate between the different evolutionary scenarios for various reasons: they are difficult to constrain (they cause second-order effects in the gravitational waveform), their physical models are still rather poor (for instance the details on the angular momentum transport) and the core-collapse supernova may act as a random variable, further changing both the amplitude and the orientation of the spin. \cite{GWTC-3_interpretation}

At the end of the third observing run, the black hole population exhibits a preference for low-spinning black holes $\chi \lesssim 0.4$, with a peak distribution at $\chi \sim 0.2$ and a large tail at higher values. Separate analysis of the fastest ($\chi_A$) and slowest ($\chi_B$) spinning components of the binary revealed that the rapid-spinning components are still quite slow $\chi_A \sim 0.4$ while the slow-spinning components are concentrated below $\chi_B \sim 0.2$, as expected. Posterior distributions for the spin magnitudes are shown in Fig.\ \ref{fig:spinmagnitude}.

The effective spin parameter $\chi_{eff}$ distribution is a Gaussian centered in $\chi_{eff, \mu} = 0.06_{-0.05}^{0.04}$. The distribution peak is compatible with $\chi_{eff} = 0$, indicating spins misaligned with the orbital angular momentum. In particular, detailed analysis require the existence of negative effective spins $\chi_{eff} < 0$, related to polar angles $\theta \geq 90^\circ $ and thus to black holes anti-aligned with the orbital angular momentum that likely formed in dynamically active environments. Moreover, data from GWTC-3 exhibit a mild anti-correlation between the effective spin parameter $\chi_{eff}$ and the mass ratio $q=M_2/M_1$, with $M_1 \geq M_2$. If confirmed, the anti-correlation would challenge the usual evolutionary pathaways for the binary black hole progenitors. 
...
\begin{figure}
	...
	\end{minipage}
	\caption{\emph{Left:} Spin magnitudes posterior distribution of all the merging black holes at the end of the third observing run O3 (in gray) compared to the 90 \% credible intervals for the sample limited at the end of O2 (in blue). \emph{Right:} Separate posterior distributions within 90 \% credible bounds of the fastest ($\chi_A$, in blue) and slowest ($\chi_B$, in green) black holes in the binaries. The new observations indicate a preference for slow-spinning black holes $\chi \lesssim 0.4$, with a fast-spinning component still rather slow $\chi_A \sim 0.4$ \cite{GWTC-3_interpretation}} \label{fig:spinmagnitude}
\end{figure}
...
\begin{figure}
	...
	\end{minipage}
	\caption{Spin magnitude (\emph{left}) and misalignment with respect to the orbital angular momentum  (\emph{right}) of the merging black holes at the end of the third observing run O3 (in gray) compared to the 90 \% credible intervals for the sample limited at the end of O2 (in blue). The new observations indicate a preference for low-spin magnitudes $\chi \lesssim 0.4$ and isotropically oriented spins. \cite{GWTC-3_interpretation}} \label{fig:spinorientation}
\end{figure}
