...
#SEVNPATH
#-------------------------------
SEVN="~/Scrivania/sevn/SEVN2-3.0.0-Spindevel_RLO" #Complete path to the SEVN folder
...
#-------------------------------
if [ "$cmdargument" = "" ]; then
  LISTBIN="${SEVN}/run_scripts/listStar.dat" #Complete path to input file (list of binaries or single stars)
else
  LISTBIN=$cmdargument
fi
...
SCOL="Worldtime:Mass:MHE:MCO:Radius:RHE:RCO:Luminosity:Temperature:Hsup:PhaseBSE:Phase:RemnantType" #Additional columns to print in the output file for single stellar evolution runs. Default is empty, but any property of single stars can be added (check names in the Property class)
